...
def GetTrainingText(text_file):
    with open(text_file) as train_file:
...
def GenCountsDict(train_string, n):
    counts_dict = {}
    for i in range(len(train_string)):
        if i + n <= len(train_string):
            ngram = train_string[i:(i+n)]
            if ngram in counts_dict:
                counts_dict[ngram] = counts_dict[ngram] + 1
            else:
                counts_dict[ngram] = 1
    return counts_dict


def GenCountsDict_n_min_one(train_string, n):
    counts_dict = {}
    for i in range(len(train_string)):
        if i + (n - 1) <= len(train_string):
            ngram = train_string[i:(i+(n - 1))]
            if ngram in counts_dict:
                counts_dict[ngram] = counts_dict[ngram] + 1
            else:
                counts_dict[ngram] = 1
...
def GetVocab(train_string):
    vocab = []
    for i in range(len(train_string)):
        if train_string[i] not in vocab:
            vocab.append(train_string[i])
        else:
            continue
    return vocab


def GenModel(counts_dict, counts_n_min_one, k, vocab, log_prob):
    probs = {}
    for x in counts_dict:
        numer = counts_dict[x]
        denom = counts_n_min_one[x[:(n - 1)]]
#       add k smoothing
        numer += k
        denom += (k*(len(vocab)))
        raw_prob = (numer / denom)
#         print(x, raw_prob)
        if log_prob == True:
            probs[x] = math.log(raw_prob)
        else:
#             prob = raw_prob
            probs[x] = raw_prob
#             probs[x] = (numer, denom, raw_prob, math.log(raw_prob))
#         print(x, raw_prob)
#         print(x, raw_prob, probs[x])
    return probs
...
# print(logprob, type(logprob))
# print(vocab[:10])
# print(len(vocab))
# pp.pprint(counts_dict)
# print(len(counts_dict))
# pp.pprint(counts_n_min_one)
# pp.pprint(model)
